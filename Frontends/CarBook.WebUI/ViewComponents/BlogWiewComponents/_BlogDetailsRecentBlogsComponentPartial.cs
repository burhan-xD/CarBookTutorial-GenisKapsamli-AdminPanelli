...
namespace CarBook.WebUI.ViewComponents.BlogWiewComponents
{
    public class _BlogDetailsRecentBlogsComponentPartial:ViewComponent
    {
        ...
        public async Task<IViewComponentResult> InvokeAsync()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7184/api/Blogs/GetLastThreeBlogsWithAuthorList");
            if (responseMessage.IsSuccessStatusCode)
            {
                var jsonData = await responseMessage.Content.ReadAsStringAsync();
                var values = JsonConvert.DeserializeObject<List<ResultLastThreeBlogsWithAuthor>>(jsonData);
                return View(values);
            }
            return View();
        }
